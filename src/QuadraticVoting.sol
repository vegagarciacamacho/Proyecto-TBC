...
contract QuadraticVoting {
    ...
    function getSignalingProposals() external view returns (uint256[] memory) {
        return _getProposalsByStatus(ProposalStatus.Pending, true);
    }
    ...
    function closeVoting() external {
        ...
        totalBudget = 0;
        if (remaining > 0) {
            // Sustituimos .transfer por .call como pide el manual
    ...
    function _returnTokensToVoters(uint256 _id) internal {
        ...
        for (uint256 j = 0; j < voters.length; j++) {
            uint256 votes = userVotesInProposal[_id][voters[j]];
            if (votes > 0) {
                uint256 tokens = votes * votes;
                userVotesInProposal[_id][voters[j]] = 0;
                token.transfer(voters[j], tokens);
            }
        }
    }
